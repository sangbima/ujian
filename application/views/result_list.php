...
<div class="container">
    ...
    <div class="row">
        <div class="col-md-12">
            <form id="generateReport" method="post" action="<?php echo site_url('result/generate_report/');?>">
                <div class="input-group">
                    <select class="input-special" name="quid">
                        <option value="0"><?php echo $this->lang->line('select_quiz');?></option>
                        <?php foreach($quiz_list as $qk => $quiz){ ?>
                        <option value="<?php echo $quiz['quid'];?>"><?php echo $quiz['quiz_name'];?></option>
                        <?php } ?>
                    </select>
                    <select class="input-special" name="gid">
                        <option value="0"><?php echo $this->lang->line('select_group');?></option>
                        <?php foreach($group_list as $gk => $group){ ?>
                        <option value="<?php echo $group['gid'];?>"><?php echo $group['group_name'];?></option>
                        <?php } ?>
                    </select>
                    <input class="input-special" type="text" name="date1" value="" placeholder="<?php echo $this->lang->line('date_from');?>">
                    <input class="input-special" type="text" name="date2" value="" placeholder="<?php echo $this->lang->line('date_to');?>">
                    <button class="btn btn-info btn-special" type="submit"><?php echo $this->lang->line('generate_report');?></button>
                </div>
            </form>
        </div>
    </div>
        ...
        <div class="col-md-12">
            ...
            <table class="table table-bordered">
                ...
                <tr>
                    ...
                    <td>
                        <a href="<?php echo site_url('result/view_result/'.$val['rid']);?>" class="btn btn-success btn-xs btn-special" ><i class="fa fa-eye"></i></a>
                        <?php if($logged_in['su']=='1'){ ?>
                        <a href="javascript:remove_entry('result/remove_result/<?php echo $val['rid'];?>');" class="btn btn-danger btn-xs btn-special"><i class="fa fa-remove"></i></a>
                        <?php } ?>
                    </td>
                </tr>
                <?php } ?>
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <a href="<?php echo site_url('result/index/'.$back.'/'.$status);?>"  class="btn btn-primary btn-special"><?php echo $this->lang->line('back');?></a>
            &nbsp;&nbsp;
            <?php $next=$limit+($this->config->item('number_of_rows')); ?>
            <a href="<?php echo site_url('result/index/'.$next.'/'.$status);?>"  class="btn btn-primary btn-special"><?php echo $this->lang->line('next');?></a>
        </div>
    </div>
</div>